...
def changeOriantation():
    ...
    cX, cY = findCenterOfMass()

def findCenterOfMass():
    ret,thresh = cv2.threshold(img_gray,127,255,0) # convert gray scall image in to binary image
...
def fingerTipsFind():
    ...
    print(fingerTips_x_array) 
    print(fingerTips_y_array) 
    c_x, c_y = findCenterOfMass()
    for coun in range(len(fingerTips_x_array)):
        # print(fingerTips_y_array[coun], fingerTips_x_array[coun])
        cv2.circle(img_rgb, (fingerTips_y_array[coun], fingerTips_x_array[coun]), 6, (0, 0, 255), 2)
        cv2.line(img_rgb, (c_x, c_y), (fingerTips_y_array[coun], fingerTips_x_array[coun]), (0, 255, 0), thickness=1, lineType=8)
